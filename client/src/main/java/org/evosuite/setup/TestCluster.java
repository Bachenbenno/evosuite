					...
					if (compatibleParameters) {
						...
						for (GenericAccessibleMember<?> modifier : entry.getValue()) {
							...
							if (!modifier.getOwnerClass().isParameterizedType()) {
								...
								GenericAccessibleMember<?> newModifier = modifier.copyWithOwnerFromReturnType(newOwner);
								...
								if (newModifier.getOwnerClass().hasWildcardOrTypeVariables()) {
									...
									GenericAccessibleMember<?> concreteNewModifier = newModifier.copyWithNewOwner(concreteClass);
		...
		for (GenericAccessibleObject<?> m : environmentMethods) {
			...
			try {
				if (m.getOwnerClass().hasWildcardOrTypeVariables()) {
					GenericClass concreteClass = m.getOwnerClass().getGenericInstantiation();
					m = m.copyWithNewOwner(concreteClass);
				}
				if (m.hasTypeParameters()) {
					m = m.getGenericInstantiation();
				}
			} catch (ConstructionFailedException e) {
				logger.error("Failed generic instantiation in "+m);
				continue;
			}
